...
for (const simpson of simpsons) {
    ...
    div.innerText = `${name} -- ${surname} -- ${age} -- ${info}`
...
for (const ul1Element of arr) {
    let liEl = document.createElement('li')
    liEl.innerText = `${ul1Element}`
    ul1.append(liEl)
...
divEl1.innerText = 'hello';
divEl1.setAttribute('id', 'text')
let button = document.createElement('button')
button.innerText = 'click'
document.body.append(divEl1, button);
button.onclick = () => {
...
button2.onclick = () => {
    ...
    const genarateTable = (raws,columns,text) => {
